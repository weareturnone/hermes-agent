...
# Persisted gateway transcripts are replayed into future model context.  Large
# tool outputs can otherwise bloat SQLite session history before context
# compression has a chance to run.
...
def compact_large_tool_result_for_persistence(
    ...
    *,
    ...
    """Return a persistence-safe copy of *message*.

    Small tool outputs are preserved byte-for-byte.  Large tool/function result
    content is replaced with a compact JSON metadata placeholder before writing
    gateway session history, preventing raw tool payloads from forcing premature
    context compaction on later Telegram/gateway turns.
    """
...
class SessionStore:
    ...
    def rewrite_transcript(self, session_id: str, messages: List[Dict[str, Any]]) -> None:
        ...
        # SQLite: replace atomically so a mid-rewrite failure does not leave
        # the session half-empty in the DB.
        if self._db:
            try:
