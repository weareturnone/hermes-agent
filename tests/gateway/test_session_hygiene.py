...
The hygiene system follows the agent compression threshold by default:
  compression.threshold × model context length
Operators can override only the gateway trigger with
  compression.hygiene_threshold × model context length
when they intentionally want different CLI and messaging behavior.
"""
...
async def _observe_hygiene_compression(
    monkeypatch,
    tmp_path,
    *,
    config,
    ...
    if isinstance(model_config, str):
        ...
        configured_provider = model_config.get("provider") or ""
    effective_runtime = {
        "provider": configured_provider,
        "api_key": "test-only-no-network",
        **(runtime or {}),
    }
    ...
    entry.last_prompt_tokens = stored_tokens
    ...
    monkeypatch.setattr(
        ...
        lambda: {"api_key": "test-only-no-network"},
    )
    monkeypatch.setattr(
...
@pytest.mark.asyncio
async def test_session_hygiene_honors_compression_threshold(monkeypatch, tmp_path):
    """compression.threshold controls the gateway token gate by default.

    Regression: gateway hygiene used a hard-coded 0.85 threshold even when
    the agent compressor used a higher configured threshold. A transcript
    between 85% and 95% of context must not compress when threshold=0.95.
    """
    fake_dotenv = types.ModuleType("dotenv")
    fake_dotenv.load_dotenv = lambda *args, **kwargs: None
    monkeypatch.setitem(sys.modules, "dotenv", fake_dotenv)
    ...
    history = _make_history(50, content_size=20)
    approx_tokens = estimate_messages_tokens_rough(history)
    assert 850 <= approx_tokens < 950
    ...
    assert FakeCompressAgent.last_instance is None, (
        "Gateway hygiene should honor compression.threshold=0.95 instead "
        "of compressing at the old hard-coded 0.85 threshold"
    )


@pytest.mark.asyncio
async def test_session_hygiene_uses_agent_default_when_threshold_omitted(monkeypatch, tmp_path):
    fake_dotenv = types.ModuleType("dotenv")
    fake_dotenv.load_dotenv = lambda *args, **kwargs: None
    monkeypatch.setitem(sys.modules, "dotenv", fake_dotenv)
    ...
    history = _make_history(50, content_size=10)
    approx_tokens = estimate_messages_tokens_rough(history)
    assert 500 <= approx_tokens < 850
    runner.session_store.load_transcript.return_value = history
    runner.session_store.has_any_sessions.return_value = True
    runner.session_store.rewrite_transcript = MagicMock()
    runner.session_store.append_to_transcript = MagicMock()
    ...
    monkeypatch.setattr(gateway_run, "_hermes_home", tmp_path)
    monkeypatch.setattr(gateway_run, "_resolve_runtime_agent_kwargs", lambda: {"api_key": "fake"})
    monkeypatch.setattr("agent.model_metadata.get_model_context_length", lambda *_args, **_kwargs: 1_000)
    ...
    assert FakeCompressAgent.last_instance is not None, (
        "Gateway hygiene should use the agent default threshold=0.50 when "
        "compression.threshold is omitted, not the old hard-coded 0.85"
    )


@pytest.mark.asyncio
async def test_session_hygiene_applies_model_threshold_override(monkeypatch, tmp_path):
    fake_dotenv = types.ModuleType("dotenv")
    fake_dotenv.load_dotenv = lambda *args, **kwargs: None
    monkeypatch.setitem(sys.modules, "dotenv", fake_dotenv)
    ...
    runner._resolve_session_agent_runtime = lambda **_kwargs: (
        ...
        {"provider": "openrouter", "api_key": "fake"},
    )
    runner._run_agent = AsyncMock(return_value={"final_response": "ok", "messages": [], "tools": [], "history_offset": 0, "last_prompt_tokens": 0})
    ...
    assert result == "ok"
    assert FakeCompressAgent.last_instance is not None, (
        "Gateway hygiene should apply the trinity-large-thinking model "
        "threshold override of 0.75 instead of following config threshold=0.95"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {
                "model": {
                    "default": "openrouter/glm-5.2-1m-preview",
                    "provider": "openrouter",
                    "context_length": 1_000_000,
                },
                "compression": {
...
async def test_session_hygiene_uses_same_effective_agent_boundary(
    monkeypatch, tmp_path, case, config, context_length, boundary, runtime
):
    observations = []
    for stored_tokens in (boundary - 1, boundary):
        observations.append(
            await _observe_hygiene_compression(
                monkeypatch,
                tmp_path,
                config=config,
...
@pytest.mark.asyncio
async def test_explicit_hygiene_threshold_is_the_only_gateway_override(
    monkeypatch, tmp_path
):
    config = {
        "model": {"default": "generic/model", "context_length": 1_000_000},
    ...
    observations = []
    for stored_tokens in (599_999, 600_000):
        observations.append(
            await _observe_hygiene_compression(
                monkeypatch,
                tmp_path,
...
async def test_invalid_hygiene_threshold_falls_back_to_effective_agent_policy(
    monkeypatch, tmp_path, invalid_override
):
    config = {
        "model": {"default": "generic/model", "context_length": 1_000_000},
        "compression": {
            "enabled": True,
            "threshold": 0.90,
            "hygiene_threshold": invalid_override,
        },
    }

    observations = []
    for stored_tokens in (899_999, 900_000):
        observations.append(
            await _observe_hygiene_compression(
                monkeypatch,
                tmp_path,
    ...
    assert observations == [False, True], (
        f"invalid hygiene override {invalid_override!r} must fall back to the "
        f"effective 900,000-token agent boundary; observed {observations}"
    )
